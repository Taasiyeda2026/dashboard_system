...
test('notCompared rows appear in per-employee timeline sorted by date then startTime', () => {
  // Instructor 10 has: activity at 08:00, then a ביטול זמן at 09:30 on the same day,
  // then another activity at 11:00. The rendered HTML must list them in that exact order.
  const attendance = [
  ...
  // Comparison cards show date + program; the notCompared card shows its activityType + time range.
  // We verify chronological order by checking relative positions of the unique program names
  // and the notCompared card's "09:30–10:00" time range (which IS rendered in notCompared cards).
  const posA  = html.indexOf('תכנית א');    // first comparison card  (08:00)
  const posNC = html.indexOf('09:30');      // notCompared card span contains '09:30–10:00'
  const posB  = html.indexOf('תכנית ב');    // second comparison card (11:00)
  ...
  // The ביטול זמן row must carry the "לא נבדק מול פעילות" label, not a mismatch status.
  assert.ok(html.includes('לא נבדק מול פעילות'), 'notCompared row must be labelled accordingly');
  // There must be no separate "דיווחים שאינם נבדקים מול פעילות" section at the bottom.
...
test('work-hours gap is formatted as time and never labelled ק״מ', () => {
  // A comparison where attendance workHours differs from dashboard workHours.
  // diffText for the workHours field must return a time string (דקות / שעות) not ק״מ.
  ...
  const result = compareAttendanceRows(attendance, dashboard);
  const html = resultsHtml(result, '2026-05');
  // The diff for workHours must not contain ק״מ.
  assert.ok(!html.includes('ק״מ') || html.indexOf('פער ק"מ') > -1, 'workHours diff must not use ק״מ as its unit');
  // The diff must mention "דקות" or "שעות" for the work-hours discrepancy.
