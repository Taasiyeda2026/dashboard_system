...
const sql=fs.readFileSync(new URL('../supabase/migrations/20260729154248_instructor_operational_scheduling.sql',import.meta.url),'utf8');
...
test('workflow applies hard filters before route calculations and limits concurrency',()=>{assert.match(workflow,/const preliminary = rankInstructors/);assert.match(workflow,/preliminaryEligible/);assert.match(workflow,/mapWithConcurrency\(preliminaryEligible, 4/);});
