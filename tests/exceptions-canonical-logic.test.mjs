...
test('end_date different from latest meeting date is end_date_out_of_sync', () => {
  const row = activity({ end_date: '2999-06-07', date_1: '2999-06-08', date_2: '2999-06-14' });
...
test('end_date equal to latest meeting date is not end_date_out_of_sync', () => {
  const row = activity({ end_date: '2999-06-14', date_1: '2999-06-08', date_2: '2999-06-14' });
...
test('activity without start_date appears as missing_start_date', () => {
  const row = activity({ start_date: null, date_1: '2999-05-01' });
...
test('all counted exceptions are returned as display instances from one source', () => {
  const rows = [
    ...
    activity({ RowID: 'SHORT-1', activity_type: 'workshop', instructor_name: '', emp_id: '', instructor_name_2: '', emp_id_2: '', start_date: '2999-05-01', end_date: '2999-05-01', date_1: '2999-05-01' })
  ];
  const model = buildExceptionsModelFromRows(rows, '2999-05', { include_rows: true });
...
test('exception relevance follows activity_type before counts and display', () => {
  ...
  const rows = [
    activity({ RowID: 'COURSE-LATE', activity_type: 'course', status: 'פתוח', end_date: '2999-06-16', date_1: '2999-06-16' }),
    activity({ RowID: 'SUMMER-LATE', activity_type: 'course', activity_season: 'summer_2026', start_date: '2999-07-01', status: 'פתוח', end_date: '2999-09-01', date_1: '2999-09-01' }),
    activity({ RowID: 'SUMMER-EXPLICIT-NO-START', activity_type: 'course', activity_season: 'summer_2026', start_date: '', status: 'פתוח', end_date: '2999-09-01', date_1: '2999-09-01' }),
    activity({ RowID: 'SUMMER-NOTES-ONLY', activity_type: 'course', notes: 'פעילות קיץ', start_date: '2999-06-01', status: 'פתוח', end_date: '2999-09-01', date_1: '2999-09-01' }),
    ...shortTypes.flatMap((activity_type) => [
      activity({ RowID: `${activity_type}-late`, activity_type, status: 'פתוח', end_date: '2999-06-16', date_1: '2999-06-16' }),
      ...
      activity({ RowID: `${activity_type}-ended-open`, activity_type, status: 'פתוח', end_date: '2000-01-01', date_1: '2000-01-01' })
    ])
  ];

  const model = buildExceptionsModelFromRows(rows, '2999-05', {
    include_rows: true,
    lateEndDateThreshold: '2999-06-15'
  });
...
test('late end date threshold creates only approved open activity exception', () => {
  const openLate = activity({ RowID: 'LATE-OPEN', status: 'פתוח', end_date: '2999-06-16', date_1: '2999-06-16' });
  const closedLate = activity({ RowID: 'LATE-CLOSED', status: 'סגור', end_date: '2999-06-16', date_1: '2999-06-16' });

  const model = buildExceptionsModelFromRows([openLate, closedLate], '2999-05', {
    include_rows: true,
    lateEndDateThreshold: '2999-06-15'
  });
...
test('pending district assignment counts as missing_district only when authority exists', () => {
  ...
  const model = buildExceptionsModelFromRows([withAuthority, withoutAuthority, deleted], '2999-05', {
    include_rows: true
  });
