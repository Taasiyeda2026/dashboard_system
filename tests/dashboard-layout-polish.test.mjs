...
test('desktop dashboard uses true 80 percent internal zoom without scaling the shell', () => {
  assert.match(dashboardCss, /#app \.screen-root:has\(\.ds-dashboard-wrap\)\s*\{[\s\S]*zoom:\s*1;/);
  assert.match(dashboardCss, /#app \.ds-dashboard-wrap\s*\{[\s\S]*width:\s*100%;[\s\S]*max-width:\s*1180px;[\s\S]*zoom:\s*0\.8;/);
