...
test('real Postgres Israa RPC flow enforces permission, group idempotency, E ownership and the Y guard', async (t) => {
  ...
  try {
    ...
    await client.query(`insert into proposal_agreement_items values ($1,$2,'course','A-1','G-1','קורס בדיקה',8,9000,1000)`, [IDS.item, IDS.proposalE]);
    await client.query(`insert into israa_program_tracking (id,proposal_agreement_id,proposal_items,authority,authority_id,school_name,school_id,contact_person,phone,email,selected_activity_drafts)
      values ($1,$2,$3,'רשות',10,'בית ספר',20,'איש קשר','050','a@example.test',$4)`, [IDS.tracking, IDS.proposalE, JSON.stringify([{ proposal_item_id: IDS.item }]), JSON.stringify([{ proposal_item_id: IDS.item, program_name: 'קורס בדיקה', quantity: 3, activity_type: 'course' }])]);
    ...
    await client.query('select public.share_israa_activity($1,$2)', [IDS.tracking, IDS.item]);
    const shared = await client.query('select activity_domain, israa_group_number, authority_id, school_id, contact_name from activities where israa_tracking_id=$1 order by israa_group_number', [IDS.tracking]);
    assert.equal(shared.rowCount, 3);
    assert.deepEqual(shared.rows.map((row) => row.israa_group_number), [1, 2, 3]);
    assert.ok(shared.rows.every((row) => row.activity_domain === 'E' && row.authority_id === '10' && row.school_id === '20' && row.contact_name === 'איש קשר'));
