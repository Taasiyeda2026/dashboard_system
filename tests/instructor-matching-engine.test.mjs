...
test('home distance above 40 km is a hard eligibility failure',()=>{
  const over=evaluateInstructor({instructor,profile,rules,activity,travel:{home:{distance_km:55,duration_minutes:50},transitions:{}}});
  assert.equal(over.eligible,false);
  assert.equal(over.score,null);
  assert.match(over.failures.join(' '),/מרחק הנסיעה לבית הספר הוא 55 ק״מ ועולה על המגבלה של 40 ק״מ/);
  const atLimit=evaluateInstructor({instructor,profile,rules,activity,travel:{home:{distance_km:40,duration_minutes:50},transitions:{}}});
  assert.equal(atLimit.eligible,true);
  const justOver=evaluateInstructor({instructor,profile,rules,activity,travel:{home:{distance_km:40.1,duration_minutes:50},transitions:{}}});
  assert.equal(justOver.eligible,false);
});
