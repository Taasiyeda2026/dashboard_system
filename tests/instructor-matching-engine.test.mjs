...
test('unavailable Google distance is explanatory data, not an exception',()=>{const result=evaluateInstructor({instructor,profile,rules,activity,travel: {const existing=[{date:'2026-08-02',start_time:'08:00',end_time:'09:55',school:'א',activity_name:'קודמת'}];const result=evaluateInstructor({instructor,profile,rules,activity,existingActivities:existing,travel:{transitions:{'2026-08-02':{previous:{distance_km:20,duration_minutes:30}}}}});assert.equal(result.eligible,false);assert.match(result.failures.join(' '),/אין זמן מעבר מספיק/);});
test('long home travel reduces travelDistance points without disqualifying',()=>{const far=evaluateInstructor({instructor,profile,rules,activity,travel: {home:{distance_km:55,duration_minutes:90},
      homeReturn: {distance_km:55,duration_minutes:90},transitions:{}}});const near=evaluateInstructor({instructor,profile,rules,activity,travel: {home:{distance_km:5,duration_minutes:10},
      homeReturn: {distance_km:5,duration_minutes:10},transitions:{}}});assert.equal(far.eligible,true);assert.ok(far.scoreBreakdown.travelDistance.points < near.scoreBreakdown.travelDistance.points);assert.equal(far.failures.length,0);});
