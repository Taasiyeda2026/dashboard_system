...
test('instructor without profile receives operational defaults',()=>{assert.deepEqual(normalizeSchedulingProfile().instruction_languages,['he']);assert.deepEqual(normalizeSchedulingProfile().education_levels,['elementary','middle_school','high_school']);assert.equal(normalizeSchedulingProfile().course_restriction_mode,'all');});
test('missing weekly rule uses Sunday-Thursday 08:00-15:00 defaults',()=>{const result=evaluateInstructor({instructor,activity:{...activity,start_time:'08:00',end_time:'15:00'},profile});assert.equal(result.eligible,true);});
