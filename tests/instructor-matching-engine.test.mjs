...
test('legacy age and restriction fields do not affect eligibility',()=>{const result=evaluateInstructor({instructor,profile:{...profile,education_levels:[],course_restriction_mode:'block_selected',course_ids:['קורס מדעים'],blocked_authorities:['חיפה'],blocked_schools:['בית ספר']},rules,activity:{...activity,education_level:'high_school',blocked_instructor_ids:['10'],allowed_instructor_ids:['11']}});assert.equal(result.eligible,true);assert.equal(result.checks.educationLevel,undefined);assert.equal(result.checks.courseEligibility,undefined);});
