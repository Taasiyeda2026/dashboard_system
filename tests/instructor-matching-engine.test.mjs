...
test('unavailable Google distance is explanatory data, not an exception',()=>{const result=evaluateInstructor({instructor,profile,rules,activity,travel:null});assert.equal(result.eligible,true);assert.deepEqual(result.warnings,[]);assert.match(result.explanation,/טרם חושב|מסלול לא ידוע/);});
...
test('long home travel reduces travelDistance points without disqualifying',()=>{
  const far=evaluateInstructor({instructor,profile,rules,activity,travel:{home:{distance_km:55,duration_minutes:90},homeReturn:{distance_km:56,duration_minutes:92},transitions:{}}});
  const near=evaluateInstructor({instructor,profile,rules,activity,travel:{home:{distance_km:5,duration_minutes:10},homeReturn:{distance_km:5.5,duration_minutes:12},transitions:{}}});
  assert.equal(far.eligible,true);
  assert.ok(far.scoreBreakdown.travelDistance.points < near.scoreBreakdown.travelDistance.points);
  assert.equal(far.failures.length,0);
});

test('missing homeReturn on solo day disqualifies in final validateTravel mode',()=>{
  const result=evaluateInstructor({instructor,profile,rules,activity,travel:{home:{distance_km:5,duration_minutes:10},homeReturn:null,transitions:{}},validateTravel:true});
  assert.equal(result.eligible,false);
  assert.match(result.failures.join(' '),/חזרה לבית|מסלול נסיעה מהפעילות חזרה/);
});
