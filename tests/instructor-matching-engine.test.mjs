...
test('instructor without profile does not assume a language requirement',()=>{assert.deepEqual(normalizeSchedulingProfile().instruction_languages,[]);assert.equal(normalizeSchedulingProfile().course_restriction_mode,'all');});
test('missing weekly availability never implies availability',()=>{const result=evaluateInstructor({instructor,activity:{...activity,start_time:'08:00',end_time:'15:00'},profile});assert.equal(result.eligible,false);assert.match(result.failures.join(' '),/חסרים נתוני זמינות/);});
