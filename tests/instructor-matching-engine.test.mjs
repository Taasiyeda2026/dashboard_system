...
test('home distance above 40 km reduces distance points to zero without disqualifying',()=>{const result=evaluateInstructor({instructor,profile,rules,activity,travel:{home:{distance_km:55,duration_minutes:50},transitions:{}}});assert.equal(result.eligible,true);assert.equal(result.scoreBreakdown.distance.points,0);assert.equal(result.failures.length,0);});
