...
begin
  ...
  if v_role not in ('admin', 'operation_manager', 'activities_manager', 'finance') then
...
comment on function public.get_payroll_attendance_records(bigint[], date, date) is 'Payroll attendance bridge over Attendance V2. Admin/operations/finance can read permitted records; activities managers are restricted to their direct instructor roster.';
comment on function public.update_payroll_attendance_record(uuid, jsonb) is 'Writes manager-approved payroll corrections back to Attendance V2. Only admin/operations or the instructor direct activities manager may update; employee month locks are intentionally not used for this manager-side payroll correction path.';
