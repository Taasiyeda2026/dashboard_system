...
create table if not exists public.scheduling_travel_cache (
  origin_key text not null, destination_key text not null, distance_km numeric, duration_minutes integer,
  provider text not null default 'google', calculated_at timestamptz not null default now(), expires_at timestamptz not null default now() + interval '30 days',
  primary key (origin_key, destination_key)
);
create table if not exists public.instructor_assignment_audit (
  id uuid primary key default gen_random_uuid(), activity_id text not null, selected_emp_id text not null,
  selected_instructor_name text not null, top_recommended_emp_id text, selected_score integer, top_score integer,
  decision_type text not null check (decision_type in ('draft','approved','overridden','exception_approved')),
  reason text, selected_by uuid not null default auth.uid(), created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
create or replace function public.assign_activity_instructor(p_activity_id text, p_emp_id text, p_instructor_name text, p_top_emp_id text, p_selected_score integer, p_top_score integer, p_decision_type text, p_reason text default null)
returns public.activities language plpgsql security definer set search_path=public as $$
declare result public.activities; current_role text := public.app_current_role();
begin
  if current_role <> all(array['admin','operation_manager']) then raise exception 'scheduling_permission_denied' using errcode='42501'; end if;
  if p_decision_type not in ('draft','approved','overridden','exception_approved') then raise exception 'invalid_decision_type'; end if;
  if (p_decision_type in ('overridden','exception_approved') or (p_top_emp_id is not null and p_emp_id <> p_top_emp_id)) and nullif(btrim(p_reason),'') is null then raise exception 'scheduling_reason_required'; end if;
  select * into result from public.activities where row_id=p_activity_id for update;
  if not found then raise exception 'activity_not_found'; end if;
  if exists (select 1 from public.activities a where a.row_id<>p_activity_id and a.emp_id=p_emp_id and a.status not in ('סגור','נמחק','בוטל') and exists (select 1 from generate_series(1,35) n where to_jsonb(a)->>('date_'||n)=any(array(select to_jsonb(result)->>('date_'||m) from generate_series(1,35) m)) and a.start_time < result.end_time and result.start_time < a.end_time)) then raise exception 'scheduling_conflict_detected'; end if;
  if p_decision_type <> 'draft' then update public.activities set emp_id=p_emp_id,instructor_name=p_instructor_name where row_id=p_activity_id returning * into result; end if;
  insert into public.instructor_assignment_audit(activity_id,selected_emp_id,selected_instructor_name,top_recommended_emp_id,selected_score,top_score,decision_type,reason) values(p_activity_id,p_emp_id,p_instructor_name,p_top_emp_id,p_selected_score,p_top_score,p_decision_type,nullif(btrim(p_reason),''));
  return result;
end $$;
revoke all on function public.assign_activity_instructor(text,text,text,text,integer,integer,text,text) from public;
grant execute on function public.assign_activity_instructor(text,text,text,text,integer,integer,text,text) to authenticated;
