...
create or replace function public.assign_activity_instructor(p_activity_id text, p_emp_id bigint, p_instructor_name text, p_top_emp_id bigint, p_selected_score integer, p_top_score integer, p_decision_type text, p_reason text default null)
  ...
  if exists (
    ...
    where a.row_id<>p_activity_id
      ...
      and exists (
        ...
        where nullif(to_jsonb(a)->>('date_'||n),'') = any(array(select nullif(to_jsonb(result)->>('date_'||m),'') from generate_series(1,35) m))
          and a.start_time < result.end_time and result.start_time < a.end_time
      )
  ) then raise exception 'scheduling_conflict_detected'; end if;
  update public.activities set emp_id=p_emp_id,instructor_name=selected_instructor.full_name where row_id=p_activity_id returning * into result;
  insert into public.instructor_assignment_audit(activity_id,selected_emp_id,selected_instructor_name,top_recommended_emp_id,selected_score,top_score,decision_type,reason)
  values(p_activity_id,p_emp_id::text,selected_instructor.full_name,p_top_emp_id::text,p_selected_score,p_top_score,p_decision_type,nullif(btrim(p_reason),''));
  return result;
end $$;
