-- Unify non-overridable scheduling feasibility gates for manual assignments.
--
-- Automatic matching keeps the 40 km home-distance limit. A deliberate manual
-- choice may still exceed 40 km under the existing manual-distance policy
-- (manager approval from 60 km), but it may not bypass identity/safety facts:
-- active profile, language, gender, known routes, real overlap, explicit
-- unavailability, transition <= 20 km, raw travel time + 15 minutes, Friday/
-- Saturday policy, or the daily sequence limit.
...
comment on function public.scheduling_manual_assignment_hard_violations(text,bigint) is
  'Manual-finalization hard gates aligned with automatic scheduling safety: language/gender, known routes, transitions <=20km + 15m buffer, overlap, explicit availability and daily sequence. Home >40km remains a deliberate manual-distance exception under the existing >=60km manager-approval rule.';
