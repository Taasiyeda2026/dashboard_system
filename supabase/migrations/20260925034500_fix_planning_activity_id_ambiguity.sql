-- Fix PL/pgSQL activity_id ambiguity in shared planning persistence.
-- Keep planning semantics unchanged; only disambiguate the local variable and conflict target.
...
begin
  ...
  select
    ...
    false,
    now()
  from public.activities a where a.row_id = v_activity_id
  on conflict on constraint scheduling_planning_rows_pkey
  do update set
