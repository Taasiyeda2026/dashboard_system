-- Remove age-layer and deprecated course/list gates from scheduling server checks.
-- Keeps legacy columns in place; does not backfill or update existing rows.

drop function if exists public.save_activity_scheduling_requirements(text, text, text, text);
...
begin
  ...
  if normalized_language is not null and normalized_language not in ('he', 'ar') then raise exception 'invalid_instruction_language'; end if;
  if normalized_gender not in ('any', 'female', 'male') then raise exception 'invalid_instructor_gender'; end if;
...
revoke all on function public.save_activity_scheduling_requirements(text, text, text) from public;
grant execute on function public.save_activity_scheduling_requirements(text, text, text) to authenticated;
