-- Dynamic manager follow-up deadlines for intro feedback and observations.
-- Rules:
-- 1) intro feedback due one calendar month after the employee is created in the system;
-- 2) observation 1 due one calendar month after the instructor's first scheduled teaching date;
-- 3) observation 2 due one calendar month after observation 1 is completed.
...
begin
  ...
  select
    ...
    case
      when coalesce(usr.employee_created_at, ef.created_at) is null then null
      else (coalesce(usr.employee_created_at, ef.created_at)::date + interval '1 month')::date
...
comment on function public.get_manager_team_roster(text, text) is
  'Manager tracking roster with canonical employee-file completion flags plus dynamic due dates: intro feedback one month after employee creation, observation 1 one month after first teaching date, observation 2 one month after observation 1 completion.';
