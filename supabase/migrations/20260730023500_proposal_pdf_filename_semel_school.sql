-- GEFEN and next-year proposal PDFs use one clear, stable display name:
-- "הצעת מחיר - <סמל מוסד> - <שם בית הספר>.pdf".
...
begin
  ...
  if use_school_filename then
    new.final_pdf_file_name := concat_ws(
      ' - ',
      'הצעת מחיר',
      nullif(semel_mosad, ''),
      nullif(school_name, '')
...
comment on function public.set_proposal_final_pdf_display_name() is
  'Keeps storage keys ASCII-safe and names GEFEN/next-year proposal PDFs as quote, institution code, and school name.';
