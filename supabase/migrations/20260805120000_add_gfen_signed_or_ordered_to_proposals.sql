alter table public.proposals_agreements
  add column if not exists gfen_signed_or_ordered boolean not null default false;

comment on column public.proposals_agreements.gfen_signed_or_ordered is
  'Manual list marker for Gefen proposals: true when a signed Gefen approval or Gefen order exists.';
