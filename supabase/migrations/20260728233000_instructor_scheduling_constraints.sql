...
create table if not exists public.instructor_scheduling_profiles (
  emp_id text primary key references public.contacts_instructors(emp_id) on delete cascade,
...
create table if not exists public.instructor_availability_rules (
  ...
  emp_id text not null references public.contacts_instructors(emp_id) on delete cascade,
  ...
  updated_at timestamptz not null default now(),
  updated_by uuid default auth.uid(),
  constraint instructor_availability_rules_weekday_check check (weekday between 0 and 6),
...
revoke all on table public.instructor_scheduling_profiles from anon;
revoke all on table public.instructor_availability_rules from anon;
revoke all on table public.instructor_availability_exceptions from anon;
...
drop policy if exists instructor_scheduling_profiles_admin_operations on public.instructor_scheduling_profiles;
...
drop policy if exists instructor_availability_rules_admin_operations on public.instructor_availability_rules;
...
drop policy if exists instructor_availability_exceptions_admin_operations on public.instructor_availability_exceptions;
