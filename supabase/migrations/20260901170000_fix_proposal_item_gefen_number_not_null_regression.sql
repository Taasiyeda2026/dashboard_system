-- Fix regression: proposal_agreement_items.gefen_number is NOT NULL DEFAULT '',
-- but the legacy implementation restored by the permissions wrapper converts
-- an empty Gefen number to NULL via nullif(..., ''). Keep empty values as ''.

CREATE OR REPLACE FUNCTION public.save_proposal_agreement_items_atomic_legacy_impl(
  p_proposal_id uuid,
...
-- The public wrapper remains unchanged and continues enforcing the explicit
-- manage-proposals permission before delegating to this implementation.
