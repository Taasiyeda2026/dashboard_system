-- Keep instructor onboarding compatible with browser tabs opened before the
-- independent-instructor frontend fix was deployed.
--
-- The legacy client sent an empty p_employment_type only for the "עצמאי"
-- onboarding path. Normalize that exact legacy payload server-side so an
-- already-open tab creates the instructor correctly instead of failing with
