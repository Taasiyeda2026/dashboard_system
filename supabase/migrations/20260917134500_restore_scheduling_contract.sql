-- Restore the approved scheduling contract on the authoritative server validator.
-- Keeps manual-exception semantics intact while normal automatic/draft scheduling
-- enforces profile gender, 40 km home limit, 20 km transition limit,
-- travel time + 15 minutes, and daily sequence limits (3 long / 5 short).
...
begin
  ...
  if not found then
    ...
    -- Gender is mandatory profile data even when the activity has no gender preference.
  ...
  for meeting in select value from jsonb_array_elements(effective_rows) loop
    ...
    -- Daily sequence contract: a chain continues while the gap is <= 30 minutes.
    -- Long activities (>=80 min) allow at most 3 in a chain; shorter ones at most 5.
    previous_end := null;
    chain_count := 0;
    for session_row in
