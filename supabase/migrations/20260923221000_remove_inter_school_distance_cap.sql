-- Transition feasibility is time-based rather than distance-capped.
-- Verified routes up to 10 km require raw travel time + 10 minutes.
-- Verified routes above 10 km require raw travel time + 15 minutes.
-- There is no hard school-to-school distance ceiling; an unverified route or
-- insufficient real travel time remains a hard blocker.
...
'Automatic scheduling validation uses verified travel-time feasibility between schools, without a hard inter-school distance ceiling.';

comment on function public.scheduling_manual_assignment_hard_violations(text,bigint) is
'Manual scheduling hard gates keep verified travel-time feasibility and overlap protection, without a hard inter-school distance ceiling.';
