create table if not exists public.manager_instructor_followup (
  ...
  intro_call_completed boolean not null default false,
  ...
  police_clearance_confirmed boolean not null default false,
  updated_at timestamptz not null default now(),
  updated_by uuid default auth.uid(),
  primary key (emp_id, school_year)
);

alter table public.manager_instructor_followup enable row level security;
revoke all on table public.manager_instructor_followup from public, anon, authenticated;
...
begin
  ...
  elsif v_role in ('admin', 'operation_manager', 'domain_manager') then
...
begin
  ...
  if v_role in ('admin', 'operation_manager', 'domain_manager') then
    v_allowed := true;
...
comment on table public.manager_instructor_followup is 'Manual manager follow-up checklist keyed only by instructor emp_id and school year; instructor identity and manager assignment remain canonical in contacts_instructors.';
comment on function public.get_manager_team_roster(text, text) is 'Single manager-team roster for the manager workspace; activities_manager is always scoped to their own direct_manager roster.';
comment on function public.update_manager_instructor_followup(bigint, text, text, boolean) is 'Updates one manager follow-up flag after server-side manager/team authorization.';
