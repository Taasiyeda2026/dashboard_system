-- Mid-year / year-end feedback execution windows for school_year=2027, plus a
-- reliable first_completed_at on instructor_employee_document_status.
--
-- updated_at is refresh-time only (edge live sync stamps now() on every scan),
-- so it must NOT be used as the feedback completion moment.
-- first_completed_at stores the earliest reliable detection (prefer SharePoint
-- file createdDateTime; otherwise first false→true transition). Never invent
-- it from a later refresh of an already-completed row.

alter table public.instructor_employee_document_status
  add column if not exists first_completed_at timestamptz;

comment on column public.instructor_employee_document_status.first_completed_at is
  'Earliest reliable completion detection for the component (SharePoint file createdDateTime or first false→true). Not refresh time; updated_at remains the last sync stamp.';

-- Preserve / adopt earliest first_completed_at; clear when not completed.
-- Do not invent a timestamp when completed stays true but no value is provided.
...
drop trigger if exists trg_instructor_employee_document_status_first_completed_at
  on public.instructor_employee_document_status;

create trigger trg_instructor_employee_document_status_first_completed_at
  before insert or update on public.instructor_employee_document_status
  for each row
  execute function public.instructor_employee_document_status_first_completed_at_trg();

-- Manual component updates: set first_completed_at only on false→true when caller
-- did not supply a more specific timestamp (column default stays null).
...
begin
  ...
  insert into public.instructor_employee_document_status (
    folder_mapping_id, component_key, completed, item_count, first_completed_at, updated_at, updated_by
  )
  values (
    mapping_id,
    p_component_key,
...
comment on function public.get_manager_team_roster(text, text) is
  'Manager tracking roster with seniority intro/opening-year rules, observation due dates, and 2027 midyear/year-end feedback windows plus first_completed_at-based feedback completion timestamps.';
