...
create or replace function public.normalize_israa_activity_type(p_type text)
returns text language sql immutable set search_path = public, pg_temp as $$
  select case lower(btrim(coalesce(p_type, '')))
    when 'קורס' then 'course' when 'course' then 'course'
    when 'חוג' then 'after_school' when 'אפטרסקול' then 'after_school' when 'after_school' then 'after_school'
    when 'סדנה' then 'workshop' when 'סדנאות' then 'workshop' when 'workshop' then 'workshop'
    when 'סיור' then 'tour' when 'סיורים' then 'tour' when 'tour' then 'tour'
    when 'חדר בריחה' then 'escape_room' when 'חדרי בריחה' then 'escape_room'
    when 'escape room' then 'escape_room' when 'escape_room' then 'escape_room'
    else nullif(lower(btrim(coalesce(p_type, ''))), '')
  end
$$;
...
begin
  ...
  v_draft := jsonb_strip_nulls(jsonb_build_object(
    ...
    'activity_type', public.normalize_israa_activity_type(coalesce(nullif(p_draft->>'activity_type', ''), v_item->>'item_type')),
...
begin
  ...
  for v_group in 1..v_quantity loop
    ...
    insert into public.activities (
      ...
      coalesce(nullif(v_draft->>'price','')::numeric::bigint, round(coalesce(v_item.total_price, v_item.unit_price))::bigint),
...
begin
  ...
  update public.activities set
    ...
    emp_id_2 = case when p_changes ? 'emp_id_2' then nullif(btrim(p_changes->>'emp_id_2'),'')::bigint else emp_id_2 end,
...
-- Provenance columns may only be introduced by the SECURITY DEFINER share RPC.
-- This trigger intentionally remains SECURITY INVOKER: direct authenticated
-- activity writes therefore cannot impersonate the owner of the share function.
create or replace function public.guard_israa_provenance_write()
returns trigger language plpgsql security invoker set search_path = public, pg_temp as $$
declare v_share_owner name;
begin
  if row(new.israa_tracking_id, new.israa_source_item_id, new.israa_group_number)
       is distinct from row(old.israa_tracking_id, old.israa_source_item_id, old.israa_group_number) then
    select r.rolname into v_share_owner
    from pg_proc p join pg_roles r on r.oid = p.proowner
    where p.oid = 'public.share_israa_activity(uuid,uuid)'::regprocedure;
    if current_user <> v_share_owner then
      raise exception 'israa_provenance_write_forbidden' using errcode = '42501';
    end if;
  end if;
  return new;
end; $$;

drop trigger if exists trg_guard_israa_provenance_write on public.activities;
create trigger trg_guard_israa_provenance_write
before insert or update of israa_tracking_id, israa_source_item_id, israa_group_number
on public.activities for each row execute function public.guard_israa_provenance_write();
...
begin
  if new.israa_tracking_id is not null or new.israa_source_item_id is not null or new.israa_group_number is not null then
    if new.activity_domain <> 'E' or new.israa_tracking_id is null or new.israa_source_item_id is null
       or new.israa_group_number < 1 or new.proposal_agreement_id is null
       or not exists (
         select 1
         from public.israa_program_tracking t
         join public.proposal_agreement_items pai
           on pai.id = new.israa_source_item_id
          and pai.proposal_agreement_id = t.proposal_agreement_id
         where t.id = new.israa_tracking_id
           and t.proposal_agreement_id = new.proposal_agreement_id
           and exists (
             select 1 from jsonb_array_elements(coalesce(t.proposal_items, '[]'::jsonb)) source_item
             where source_item->>'proposal_item_id' = new.israa_source_item_id::text
           )
       ) then
