-- Manager workspace tracking is a read-only projection of the canonical employee file.
-- The employee-file document status table is the only source of completion state.
-- Legacy manager_instructor_followup flags remain stored for backward compatibility but are not read here.
...
begin
  ...
  select
    ...
    -- Backward-compatible fields, now derived only from the employee-file components.
    ...
    -- Exact employee-file components used by the manager tracking table.
...
comment on function public.get_manager_team_roster(text, text) is
  'Manager tracking roster. Identity/manager assignment come from contacts_instructors; every tracking completion flag comes only from the canonical employee-file document status for the selected school year. The manager view is read-only and excludes payroll reports.';
