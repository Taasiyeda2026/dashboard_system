...
- `supabase/migrations/20260705160000_fix_lists_list_id_default_sequence.sql` ← **⚡ דחוף: מוסיף auto-increment ל-lists.list_id (טיפול בשני מקרים: identity column → GENERATED BY DEFAULT + RESTART; plain bigint → sequence + DEFAULT). בלעדיו כל שמירת סדנה נכשלת.**
