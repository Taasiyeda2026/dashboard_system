...
function internalGroups(groups = []) {
  ...
  if (nextYear) {
    nextYear.included_group_keys = [NEXT_YEAR_COURSES_GROUP, NEXT_YEAR_WORKSHOPS_GROUP];
  }
...
function workshopRowData(row) {
  ...
  return {
    nameHtml: cells[0]?.innerHTML || name,
    quantity,
    unitPrice,
    total,
    sourceRow: row
...
function workshopTableHtml(rows, totalLabel, totalValue) {
  const body = rows.map((row) => `<tr>
    <td>${row.nameHtml}</td>
    <td>45 דקות</td>
    <td>${row.quantity.toLocaleString('he-IL')}</td>
    <td>${currencyHtml(row.unitPrice)}</td>
    <td>${currencyHtml(row.total)}</td>
  </tr>`).join('');
...
function tableGrandTotal(table) {
  const footerAmounts = Array.from(table.querySelectorAll('tfoot .pa-currency-amount, tfoot .money-amount'));
  for (let index = footerAmounts.length - 1; index >= 0; index -= 1) {
    const value = numericValue(footerAmounts[index].textContent);
...
function splitNextYearTable(table) {
  ...
  if (!hasCourseRows) {
    ...
    table.replaceWith(workshopHeading, workshopTable);
    return true;
  }

  table.dataset.nextYearWorkshopsSplit = 'yes';
  replaceCourseFooter(table, 'סה״כ קורסים', courseRowsTotal(table));
  const courseHeading = heading(documentRef, 'קורסים ותוכניות', 'pa-next-year-course-heading');
  table.parentNode.insertBefore(courseHeading, table);
  const workshopHeading = heading(documentRef, 'סדנאות', 'pa-next-year-workshop-heading');
...
function wrapSnapshotMethod(targetApi, methodName, documentRef) {
  ...
  targetApi[methodName] = async function nextYearWorkshopSnapshot(...args) {
    ...
    if (payload && typeof payload === 'object' && typeof payload.documentHtmlSnapshot === 'string') {
      args[1] = {
        ...payload,
        documentHtmlSnapshot: normalizeNextYearWorkshopHtml(payload.documentHtmlSnapshot, documentRef)
      };
    }
