...
export function buildIsraaWorkbook(sourceRows) {
  ...
  decorateSheet(main, MAIN_HEADERS, rows.length, [14,13,16,14,22,13,18,14,16,24,32,22,18,30,12,22,16,16,18,16,34,14,34], [10,11,12,14,20,22]);
  decorateSheet(items, ITEM_HEADERS, itemRows(rows).length, [14,24,34,18,20], [2]);
  for (let r = 1; r <= rows.length; r += 1) {
    for (const c of [1, 21]) if (main[XLSX.utils.encode_cell({ r, c })]?.v) main[XLSX.utils.encode_cell({ r, c })].z = 'dd/mm/yyyy';
    for (const c of [15, 18]) main[XLSX.utils.encode_cell({ r, c })].z = '₪#,##0.00';
    main[XLSX.utils.encode_cell({ r, c: 17 })].z = '0%';
    for (const c of [5, 8, 11]) if (main[XLSX.utils.encode_cell({ r, c })]) main[XLSX.utils.encode_cell({ r, c })].t = 's';
  }
  ...
  workbook.Workbook.Names = [
    { Name: 'IsraaTrackingTable', Ref: `'מעקב הצעות איסראא'!$A$1:$W$${rows.length + 1}` },
