...
const MAIN_HEADERS = [
  'מספר הצעה', 'תאריך הצעה', 'סוג פעילות', 'רשות', 'בעלות', 'בית ספר', 'סמל מוסד',
  'איש קשר', 'תפקיד', 'טלפון', 'דוא״ל', 'פירוט ההצעה', 'מספרי גפ״ן',
  'התוכנית הצפויה / בחירה נדרשת', 'שכבה', 'קבוצות / משתתפים', 'סכום ההצעה',
  'מבנה ההצעה', 'הערכת סגירה (%)', 'תחזית כספית', 'סטטוס', 'פעולה להמשך',
  'תאריך מעקב', 'הערה',
];
const ITEM_HEADERS = ['מספר הצעה', 'בית ספר', 'סוג פעילות', 'שם הפעילות', 'מספר גפ״ן', 'מספר קבוצות / כמות'];
...
const activityType = (row) => text(row.activity_type || row.activity_type_group || row.proposal_type
  || (Array.isArray(row.proposal_items) ? row.proposal_items.find((item) => text(item?.item_type))?.item_type : ''));
const contactName = (row) => text(row.contact_person || row.manager_name);
const contactRole = (row) => text(row.contact_role || row.role || row.additional_contact);

function mainRow(row) {
  const amount = number(row.total_amount);
  const probability = number(row.probability);
  return [
    text(row.quote_number), date(row.proposal_date), activityType(row), text(row.authority), text(row.ownership),
    text(row.school_name), text(row.semel_mosad), contactName(row), contactRole(row),
    text(row.phone || row.manager_phone), text(row.email || row.manager_email), text(row.program_name),
...
function itemRows(rows) {
  return rows.flatMap((row) => Array.isArray(row.proposal_items) ? row.proposal_items.map((item) => [
    text(row.quote_number), text(row.school_name), activityType(row), text(item?.program_name),
...
export function buildIsraaWorkbook(sourceRows) {
  ...
  decorateSheet(main, MAIN_HEADERS, rows.length, [14,13,14,16,14,22,13,18,14,16,24,32,22,30,12,22,16,18,16,18,16,34,14,34], [11,12,13,15,21,23]);
  decorateSheet(items, ITEM_HEADERS, itemRows(rows).length, [14,24,16,34,18,20], [3]);
  for (let r = 1; r <= rows.length; r += 1) {
    ...
    for (const c of [6, 9, 12]) if (main[XLSX.utils.encode_cell({ r, c })]) main[XLSX.utils.encode_cell({ r, c })].t = 's';
  }
  ...
  workbook.Workbook.Names = [
    ...
    { Name: 'IsraaActivityItemsTable', Ref: `'פירוט פעילויות'!$A$1:$F$${itemRows(rows).length + 1}` },
  ];
  return workbook;
}
