...
const NAV_ATTRIBUTE = 'data-summer-feedback-admin-nav-item';
...
const STYLE_ID = 'summer-feedback-admin-nav-styles';
...
async function currentUserCanManageSummerFeedback() {
...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    .shell-nav__btn[${NAV_ATTRIBUTE}] .summer-admin-nav__icon {
      width: 18px;
      text-align: center;
      font-size: 16px;
      line-height: 1;
...
function createSummerAdminNavItem(doc = document) {
  const button = doc.createElement('button');
  button.type = 'button';
  button.className = 'shell-nav__btn';
  button.setAttribute(NAV_ATTRIBUTE, 'true');
  button.setAttribute('aria-label', 'קיץ');
  button.title = 'משובי הקיץ של הצוות החינוכי';
  button.innerHTML = '<span class="summer-admin-nav__icon" aria-hidden="true">☀️</span><span>קיץ</span>';
...
function injectSummerAdminNavItem(host) {
  if (!host?.isConnected || host.querySelector(`[${NAV_ATTRIBUTE}]`)) return;
  const item = createSummerAdminNavItem(host.ownerDocument || document);
...
function removeSummerAdminNavItem(root = document) {
...
async function enhanceSidebar() {
  ...
  const allowed = await currentUserCanManageSummerFeedback();
  if (!host.isConnected) return;
  if (!allowed) {
    removeSummerAdminNavItem(document);
    return;
  }

  ensureStyles();
  injectSummerAdminNavItem(host);
}
...
function initializeSummerFeedbackAdminNavigation() {
...
initializeSummerFeedbackAdminNavigation();
