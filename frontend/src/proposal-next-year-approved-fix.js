...
const INSTALL_KEY = Symbol.for('taasiyeda.nextYearMixedProposalTables.v2');
...
function rowKind(row) {
  ...
  // Item metadata is more specific than the generic `next_year` group.
  // A workshop that lives under the next-year parent must never be classified as a course.
...
function normalizeDocument(documentRoot) {
  ...
  // Intervene only in the mixed case. Single-type proposals stay under the native renderer.
