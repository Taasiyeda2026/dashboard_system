import { courseSchedulingScreen } from './screens/course-scheduling.js';
import { isCourseSchedulingInterfaceEligible } from './screens/shared/activity-scheduling-eligibility.js';
...
function restoreSnapshot(state, courses) {
  ...
  try {
    ...
    const courseById = new Map(courses.map((course) => [idOf(course), course]));
    state.courseSchedulingResults = snapshot.results.filter((result) => {
      const course = courseById.get(idOf(result?.course));
      return course && !text(course.emp_id) && !text(course.draft_emp_id);
    });
