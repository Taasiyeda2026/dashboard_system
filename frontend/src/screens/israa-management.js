...
function progCellEdit(key, value, col, actNames) {
  ...
  if (col.type === 'status') {
    return `<select class="israa-inp" name="${key}">
...
function progRowHtml(row, editingId, editData, expandedId, actNames) {
  ...
  const mainCells = PROG_MAIN_COL_DEFS.map((col) => {
    const content = ed
      ? progCellEdit(col.key, editData[col.key] ?? row[col.key], col, actNames)
      : progCellView(row, col);

    return `<td class="israa-cell${col.key === 'notes' ? ' israa-cell--notes' : ''}">${content}</td>`;
  }).join('');
  ...
  if (ed) {
    detailContent = PROG_CONTACT_COL_DEFS.map((col) => {
      const input = progCellEdit(col.key, editData[col.key] ?? row[col.key], col, actNames);
      return `<div class="prog-detail__field"><span class="prog-detail__lbl">${escapeHtml(col.label)}:</span>${input}</div>`;
    }).join('');
  } else {
    const parts = PROG_CONTACT_COL_DEFS.map((col) => {
      const val = String(row[col.key] ?? '');
      return val
        ? `<span class="prog-detail__item"><span class="prog-detail__lbl">${escapeHtml(col.label)}:</span> <span>${escapeHtml(val)}</span></span>`
        : '';
    }).join('');
  ...
  return mainRow + `<tr class="prog-detail-row" data-detail-for="${escapeHtml(row.id)}">
    <td colspan="${PROG_MAIN_COL_DEFS.length + 1}" class="prog-detail-cell${ed ? ' prog-detail-cell--edit' : ''}">
      ${detailContent}
    </td>
  </tr>`;
}
...
function progNewRowHtml(newData, actNames) {
  ...
  const contactCells = PROG_CONTACT_COL_DEFS.map((col) => {
    const input = progCellEdit(col.key, newData[col.key], col, actNames);
    return `<div class="prog-detail__field"><span class="prog-detail__lbl">${escapeHtml(col.label)}:</span>${input}</div>`;
  }).join('');

  return `<tr class="israa-row israa-row--new" data-row-id="__new__">
      ${mainCells}
      <td class="israa-cell israa-cell--actions">
    ...
    <tr class="prog-detail-row">
      <td colspan="${PROG_MAIN_COL_DEFS.length + 1}" class="prog-detail-cell prog-detail-cell--edit">
        ${contactCells}
      </td>
    </tr>`;
}
...
function progTableHtml(rows, editingId, editData, addingNew, newData, error, expandedId, actNames) {
  ...
  const body = [
    !rows.length && !addingNew
      ? `<tr><td colspan="${colCount}" class="israa-empty">אין רשומות. לחצי "+ הוספת שורה" להתחיל.</td></tr>`
      : '',
    ...
    <div class="prog-section">
      ...
      <div class="israa-table-wrap">
        <table class="israa-table prog-table" dir="rtl">
          <thead>
            <tr>${hdr}<th class="israa-th israa-th--actions">פעולות</th></tr>
          </thead>
          <tbody>${body}</tbody>
        </table>
      </div>
...
function simGoalLines(calc) {
  if (calc.overGoal || calc.balance <= 0) {
    ...
    if (calc.overAmt > 0) {
      lines.push(`מעבר ליעד: <strong class="sim-goal-opts__highlight">+${fmtIls(calc.overAmt)}</strong>`);
    }
...
function simCardsHtml(calc) {
  ...
  return `<div class="sim-cards">
      <div class="sim-card">
        ...
        <div class="sim-card__val sim-card__val--goal">${fmtIls(SIM_GOAL)}</div>
      </div>
      <div class="sim-card">
        <div class="sim-card__lbl">סה"כ הכנסות</div>
        <div class="sim-card__val">${fmtIls(total)}</div>
      </div>
      <div class="sim-card">
        <div class="sim-card__lbl">נותר ליעד</div>
        <div class="sim-card__val${overGoal ? ' sim-card__val--over' : ''}">${overGoal ? fmtIls(0) : fmtIls(remaining)}</div>
      </div>
      <div class="sim-card${overGoal ? ' sim-card--highlight' : ''}">
        <div class="sim-card__lbl">התקדמות</div>
        <div class="sim-card__val${overGoal ? ' sim-card__val--over' : ''}">${pctStr}${overGoal ? ' 🎉' : ''}</div>
      </div>
...
function simProgressHtml(calc) {
  ...
  return `<div class="sim-progress" dir="rtl">
      ...
      <div class="sim-progress__track">
        <div class="sim-progress__fill${calc.overGoal ? ' sim-progress__fill--over' : ''}" style="width:${fill}%"></div>
      </div>
    </div>`;
}
...
function simRowHtml(row, editingId, editData) {
  ...
  const payerRaw = ed ? (editData.payer_name ?? row.payer_name ?? '') : (row.payer_name ?? '');
  const amtRaw = ed ? (editData.amount ?? row.amount ?? '') : (row.amount ?? '');
  const comm = Number(amtRaw) > 0 ? Number(amtRaw) * 0.1 : null;

  const payerV = escapeHtml(String(payerRaw ?? ''));
  ...
  const payerCell = ed
    ? `<td class="israa-cell sim-cell--payer"><input class="israa-inp" name="payer_name" type="text" value="${payerV}" /></td>`
    : `<td class="israa-cell sim-cell--payer">${payerV}</td>`;
  ...
  const actions = ed
    ? `<td class="israa-cell israa-cell--actions">
        <button class="israa-btn israa-btn--save" data-sim-save="${escapeHtml(row.id)}" title="שמירה">💾</button>
        <button class="israa-btn israa-btn--cancel" data-sim-cancel="${escapeHtml(row.id)}" title="ביטול">✕</button>
      </td>`
    : `<td class="israa-cell israa-cell--actions">
        <button class="israa-btn israa-btn--edit" data-sim-edit="${escapeHtml(row.id)}" title="עריכה">✏️</button>
        <button class="israa-btn israa-btn--del" data-sim-del="${escapeHtml(row.id)}" title="מחיקה">🗑️</button>
      </td>`;
...
function simNewRowHtml(newData) {
  return `<tr class="israa-row israa-row--new">
      <td class="israa-cell sim-cell--date">
        <input class="israa-inp" name="revenue_date" type="date" value="${escapeHtml(String(newData.revenue_date ?? ''))}" />
      </td>
      <td class="israa-cell sim-cell--payer">
        <input class="israa-inp" name="payer_name" type="text" value="${escapeHtml(String(newData.payer_name ?? ''))}" placeholder="גורם משלם" />
      </td>
      <td class="israa-cell sim-cell--amt">
        <input class="israa-inp sim-inp--amt" name="amount" type="number" min="0" step="1" value="${escapeHtml(String(newData.amount ?? ''))}" placeholder="0" />
      </td>
...
function simPanelHtml(rows, editingId, editData, addingNew, newData, error, loading, collab) {
  ...
  const body = [
    !rows.length && !addingNew
      ? `<tr><td colspan="5" class="israa-empty">אין הכנסות. לחצי "+ הוספת הכנסה" להתחיל.</td></tr>`
      : '',
    ...
    (Array.isArray(tracking.selected_activity_drafts) ? tracking.selected_activity_drafts : []).forEach((draft) => {
      ...
      for (let groupNumber = 1; groupNumber <= quantity; groupNumber += 1) {
        const overrides = draft?.group_overrides && typeof draft.group_overrides === 'object' && !Array.isArray(draft.group_overrides)
          ...
          : null;
        const groupDraft = { ...draft, ...(overrides && typeof overrides === 'object' ? overrides : {}) };
        delete groupDraft.group_overrides;
        const draftRow = {
          ...groupDraft,
          row_id: `israa-draft|${tracking.id}|${draft.proposal_item_id}|${groupNumber}`,
          source_sheet: 'activities',
          activity_name: groupDraft.activity_name || groupDraft.program_name,
          activity_no: groupDraft.activity_no || groupDraft.gefen_number,
          school: tracking.school_name,
          authority: tracking.authority,
          class_group: groupDraft.class_group || `קבוצה ${groupNumber}`,
          israa_group_number: groupNumber,
          israa_total_groups: quantity,
...
export const israaManagementScreen = {
  ...
  bind({ root, state, api }) {
      ...
      const israApi = {
        ...
        saveActivity: async (payload) => {
          ...
          if (rowId.startsWith('israa-draft|')) {
            const [, trackingId, proposalItemId, rawGroupNumber] = rowId.split('|');
            const groupNumber = Math.max(1, Number(rawGroupNumber || 1) || 1);
            const result = await api.saveIsraaActivityGroupDraft(trackingId, proposalItemId, groupNumber, payload.changes || {});
    ...
    mgmt.addEventListener('click', async (e) => {
      ...
      if (t.closest('[data-israa-action="export-csv"]')) {
        exportToCsv(_rows);
        return;
      }
      ...
      if (t.closest('[data-israa-cancel]')) {
        _editingId = null;
        _editData = {};
        repaint();
        return;
      ...
      if (t.closest('[data-israa-cancel-new]')) {
        _addingNew = false;
        _newData = {};
        repaint();
        return;
      ...
      if (t.closest('[data-sim-cancel-new]')) {
        _simAddingNew = false;
        _simNewData = {};
        repaint();
        return;
      ...
      if (simSaveBtn) {
        ...
        simSaveBtn.disabled = true;
        simSaveBtn.textContent = '…';
        ...
        try {
          ...
          _simError = e.message || 'שגיאה בשמירה';
        }
      ...
      if (simSaveNewBtn) {
        ...
        simSaveNewBtn.disabled = true;
        simSaveNewBtn.textContent = '…';
        ...
        try {
          ...
          _simError = e.message || 'שגיאה בהוספה';
        }
      ...
      if (simDelBtn) {
        ...
        try {
          ...
          _simError = e.message || 'שגיאה במחיקה';
        }
    ...
    mgmt.addEventListener('input', (e) => {
      ...
      if (resultEl) {
        resultEl.innerHTML = simGoalLines(simCalc(_simRows, val));
      }

      const formattedEl = mgmt.querySelector('[data-sim-collab-formatted]');
      if (formattedEl) {
        formattedEl.innerHTML = fmtIls(val);
      }
    });
