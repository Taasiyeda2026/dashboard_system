...
export const myDataScreen = {
  ...
  render(data, { state } = {}) {
    ...
    const body = preparedRows.map((row) => {
      ...
      const searchHay = columns.map((column) => String(displayCellValue(row, column))).join(' ');
      return `<tr class="ds-data-row" data-list-item data-search="${escapeHtml(searchHay)}" data-filter="${escapeHtml(rawType)}" data-status="${escapeHtml(status.key)}" data-month="${escapeHtml(rowMonth)}" data-responsible="${responsible ? 'yes' : 'no'}" data-row-id="${escapeHtml(row.RowID)}" role="button" tabindex="0">${columns.map((column) => {
        if (column === 'completion_approval_status') return `<td>${statusChipHtml(status)}</td>`;
        return `<td>${escapeHtml(displayCellValue(row, column))}</td>`;
      }).join('')}</tr>`;
    });
    ...
    const tableBlock = preparedRows.length === 0
      ...
      : dsTableWrap(`<table class="ds-table ds-table--interactive ds-table--ops">
          <thead><tr>${columns.map((column) => {
            const labels = { start_date: 'תאריך', activity_day: 'יום', activity_hours: 'שעות', authority: 'רשות', school: 'בית ספר', grade: 'שכבה', activity_name: 'שם פעילות', participants_count: 'משתתפים', completion_approval_status: 'סטטוס' };
            return `<th data-col="${escapeHtml(column)}">${escapeHtml(labels[column] || hebrewColumn(column))}</th>`;
          }).join('')}</tr></thead>
  ...
  bind({ root, data, ui, state }) {
    ...
    ui?.bindInteractiveCards(root, (action) => {
      ...
      const hit = rowById.get(String(action.slice('mydata:'.length)));
      if (hit) openActivityDetail(hit);
    });
  }
};
