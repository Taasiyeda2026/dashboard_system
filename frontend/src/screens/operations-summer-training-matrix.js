...
function addStyle(){
  ...
  s.textContent=`
    ...
    .ops-trx-h{padding:14px 16px;background:#f8fbfd;border-bottom:1px solid #e2e8f0}.ops-trx-h h2{margin:0;font-size:18px;color:#0f172a}.ops-trx-h p{margin:3px 0 0;color:#64748b;font-size:12px;line-height:1.45}
...
function buildModel(activities,trainings){
  const trained=new Set(),assigned=new Map(),workshops=[],instructors=[];
  const addWorkshop=(v)=>{if(clean(v)&&!isTamir(v))workshops.push(clean(v));};
  ...
  return{workshops:sortHe(workshops),instructors:sortHe(instructors),trained,assigned};
}
...
function html(model){
  ...
  const table=model.workshops.length&&model.instructors.length?`<div class="ops-trx-wrap"><table style="min-width:${width}px;width:${width}px"><thead>${head}</thead><tbody>${body}</tbody></table></div>`:'<div class="ops-trx-msg">לא נמצאו הכשרות או שיבוצים להצגה</div>';
  return `<section class="ops-trx" dir="rtl"><div class="ops-trx-h"><h2>הכשרות קיץ</h2><p>סדנאות מול מדריכים. ✓ מציין הכשרה כללית גם אם אין שיבוץ כרגע. ! מציין שיבוץ בטווח ללא הכשרה. טווח השיבוצים לסדנאות: ${RANGE_LABEL}. סדנאות תמיר אינן מוצגות בעמוד זה.</p></div>${table}<div class="ops-trx-legend"><span><strong class="ops-trx-mark ops-trx-ok">✓</strong> עבר הכשרה לסדנה</span><span><strong class="ops-trx-mark ops-trx-warn">!</strong> משובץ ללא הכשרה</span><span>תא ריק = אין הכשרה ואין שיבוץ בטווח</span></div></section>`;
}
