...
export const instructorMyActivitiesScreen = {
  ...
  render(data, { state } = {}) {
    ...
    const desktop = `<div class="portal-activities-desktop">${dsTableWrap(`<table class="ds-table ds-table--interactive"><thead><tr><th>תאריך</th><th>שעות</th><th>פעילות</th><th>סוג</th><th>רשות</th><th>בית ספר</th><th>שכבה</th><th>איש קשר</th></tr></thead><tbody>${body}</tbody></table>`)}</div>`;
    ...
    return dsScreenStack(`<section class="instructor-area instructor-area--table">${dsPageHeader('הפעילויות שלי', 'כל הפעילויות שמשויכות אליך')}${dsCard({ title: 'הפעילויות שלי', badge: String(rows.length), body: presentation, padded: !rows.length })}</section>`);
  },
