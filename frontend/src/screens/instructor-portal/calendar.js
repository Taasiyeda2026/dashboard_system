...
function dayDrawerHtml(events, date) {
  ...
  return `<div class="instr-day-drawer"><h3>${escapeHtml(formatDateHe(date) || date)}</h3>${events.map((event) => `<article class="instr-activity-card${event.kind === 'instructor-activity' ? ' is-instructor-activity' : ''}"><div><strong>${escapeHtml(event.displayTitle)}</strong>${event.kind === 'instructor-activity' ? `<small>מפגש ${event.meetingNo}${event.school ? ` · ${escapeHtml(event.school)}` : ''}</small>` : ''}</div></article>`).join('')}</div>`;
}
