...
export function organizationalCalendarGridHtml(data, month = selectedMonth) {
  ...
  const slots = Array.from({ length: Math.ceil((firstWeekday + days) / 7) * 7 }, (_, index) => {
    ...
    return `<div class="ds-cal-slot-hit" data-calendar-date="${date}">${dsInteractiveCard({ action: `organization-day|${date}`, title: String(day), subtitle: organizationalCalendarDayLabel(events), meta: events.length > 1 ? `${events.length} אירועים` : '', variant: 'day-cell', extraClass: events.length ? 'is-school-calendar-day' : '' })}</div>`;
  }).join('');
...
function dayDrawerHtml(events, date) {
  ...
  return `<div class="instr-day-drawer"><h3>${escapeHtml(formatDateHe(date) || date)}</h3>${events.map((event) => `<article class="instr-activity-card"><div><strong>${escapeHtml(event.displayTitle)}</strong></div></article>`).join('')}</div>`;
}
