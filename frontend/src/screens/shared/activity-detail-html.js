...
function headerHtml(row, { mode = 'single', summaryDate = '', exportAction = true, instructorLimited = false } = {}) {
  ...
  }
  {
    ...
    const metaTags = [
      ...
      domainVal && !instructorLimited ? `<span class="activity-drawer__meta-tag activity-drawer__meta-tag--domain">${escapeHtml(domainVal)}</span>` : '',
...
export function activityWorkDrawerHtml(row, opts = {}) {
  const { mode = 'single', summaryDate = '', privateNote = null, settings = {}, datesLoading = false, exportAction = true, instructorLimited = false, currentInstructorIds = [], currentInstructorName = '' } = opts;
  /**
