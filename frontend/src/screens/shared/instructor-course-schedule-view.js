...
export function courseScheduleTableHtml(rows = [], { expandedDates = {} } = {}) {
  ...
  const mobile = `<div class="course-schedule-mobile">${rows.map((row) => {
    const isExpanded = Boolean(expandedDates[row.key]);
    const fields = [['סוג פעילות', row.activityType || '—'], ['רשות', row.authority], ['בית ספר / מסגרת', row.school], ['יום', row.weekday || '—'], ['שעות', row.timeRange || '—'], ['תקופה', formatWorkScheduleRangeShort(row.startDate, row.endDate) || '—'], ['שכבה', row.grade || '—'], ['מדריך/ים', row.instructorNames.join(', ')]];
