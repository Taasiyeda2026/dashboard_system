...
export function courseScheduleTableHtml(rows = [], { expandedDates = {} } = {}) {
  ...
  if (!rows.length) return dsEmptyState('לא נמצאו קורסים מוכנים לסידור עבודה בטווח הנבחר');
  ...
  const mobile = `<div class="course-schedule-mobile">${rows.map((row) => {
    const isExpanded = Boolean(expandedDates[row.key]);
    const fields = [['רשות', row.authority], ['בית ספר', row.school], ['יום', row.weekday || '—'], ['שעות', row.timeRange || '—'], ['תקופה', formatCourseScheduleRangeShort(row.startDate, row.endDate) || '—'], ['שכבה', row.grade || '—'], ['מספר מפגשים', row.sessionsCount], ['מדריך', row.instructorNames.join(', ')]];
