...
export function courseScheduleTableHtml(rows = [], { expandedDates = {}, showInstructorColumn = true } = {}) {
  const columnCount = showInstructorColumn ? 10 : 9;
  const tableRows = rows.map((row) => {
    const isExpanded = Boolean(expandedDates[row.key]);
    const instructorLabel = row.instructorNames.join(', ');
    const datesToggleLabel = 'תאריכי הפעילות';
    return `<tr>
      ...
      ${showInstructorColumn ? `<td class="ds-ops-course-col--instructor" title="${escapeHtml(instructorLabel)}">${escapeHtml(instructorLabel)}</td>` : ''}
    ...
    </tr><tr class="ds-ops-course-dates-row" data-ops-course-dates-row="${escapeHtml(row.key)}"${isExpanded ? '' : ' hidden'}><td colspan="${columnCount}">${datesListHtml(row.dates)}</td></tr>`;
  }).join('');
  if (!rows.length) return dsEmptyState('לא נמצאו פעילויות משויכות עם תאריך בטווח הנבחר');
  const desktop = `<div class="course-schedule-desktop">${dsTableWrap(`<table class="ds-table ds-table--compact ds-ops-course-schedule-table"><thead><tr><th class="ds-ops-course-col--name">שם הפעילות</th><th>סוג פעילות</th><th class="ds-ops-course-col--authority">רשות</th><th class="ds-ops-course-col--school">בית ספר / מסגרת</th>${showInstructorColumn ? '<th class="ds-ops-course-col--instructor">מדריך/ים</th>' : ''}<th class="ds-ops-course-col--weekday">יום</th><th class="ds-ops-course-col--time">שעות</th><th class="ds-ops-course-col--period">תקופת הפעילות</th><th class="ds-ops-course-col--grade">כיתה</th><th class="ds-ops-course-col--dates">תאריכי הפעילות</th></tr></thead><tbody>${tableRows}</tbody></table>`)}</div>`;
  const mobile = `<div class="course-schedule-mobile">${rows.map((row) => {
    const isExpanded = Boolean(expandedDates[row.key]);
    const fields = [['סוג פעילות', row.activityType || '—'], ['רשות', row.authority], ['בית ספר / מסגרת', row.school], ['יום', row.weekday || '—'], ['שעות', row.timeRange || '—'], ['תקופה', formatWorkScheduleRangeShort(row.startDate, row.endDate) || '—'], ['שכבה', row.grade || '—'], ...(showInstructorColumn ? [['מדריך/ים', row.instructorNames.join(', ')]] : [])];
    return `<article class="instr-activity-list-card course-schedule-mobile-card"><h3>${escapeHtml(row.name)}</h3><dl>${fields.map(([label, value]) => `<div><dt>${escapeHtml(label)}</dt><dd>${escapeHtml(String(value))}</dd></div>`).join('')}</dl><button type="button" class="ds-btn ds-btn--sm ds-ops-course-dates-toggle" data-ops-course-dates-toggle="${escapeHtml(row.key)}" aria-expanded="${isExpanded ? 'true' : 'false'}">תאריכי הפעילות</button><div class="course-schedule-mobile-dates"${isExpanded ? '' : ' hidden'}>${datesListHtml(row.dates)}</div></article>`;
  }).join('')}</div>`;
