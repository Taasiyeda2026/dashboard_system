...
export function profileHtml(row, activities, canEdit, schedulingLoaded) {
  ...
  return `<div dir="rtl" class="instructor-profile">
    <section class="instructor-profile__identity"><span class="instructor-profile__id">${escapeHtml(row.emp_id || '')}</span><p class="instructor-profile__status-line" data-instructor-status-line>${statuses.map(escapeHtml).join(' | ')}</p></section>${profileMissing.length ? `<p class="scheduling-warning"><b>חסר להשלמה:</b> ${escapeHtml(profileMissing.join(', '))}.</p>` : ''}
