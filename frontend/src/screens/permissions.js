...
export const permissionsScreen = {
  ...
  render(data, { state }) {
    const canEdit = state?.user?.role === 'admin' || state?.user?.role === 'operations_reviewer';
    ...
    const body = safeRows.map((row) => {
      const roleDisplay = canEdit
        ...
        : escapeHtml(row.display_role);

      const activeDisplay = canEdit
        ? `<label><input type="checkbox" data-active-toggle data-user-id="${escapeHtml(row.user_id)}" ${row.active === 'yes' ? 'checked' : ''} /> active</label>`
        : escapeHtml(row.active);

      const saveButton = canEdit
        ? `<button class="btn" data-save-permission data-user-id="${escapeHtml(row.user_id)}" data-full-name="${escapeHtml(row.full_name)}" data-entry-code="${escapeHtml(row.entry_code || '')}" data-default-view="${escapeHtml(row.default_view || '')}">Save</button>`
        : '';
  ...
  bind({ root, api }) {
    root.querySelectorAll('[data-save-permission]').forEach((button) => button.addEventListener('click', async () => {
