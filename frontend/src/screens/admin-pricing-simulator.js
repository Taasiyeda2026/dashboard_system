...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    .admin-pricing-simulator__assumptions-title strong {
      font-size: 11px;
      font-weight: 950;
    }
    .admin-pricing-simulator__assumptions-title span {
      color: #64748b;
      font-size: 9.5px;
    }
    ...
    .admin-pricing-simulator__assumptions-grid .admin-pricing-simulator__field:first-child input,
    .admin-pricing-simulator__assumptions-grid .admin-pricing-simulator__field:nth-child(2) input {
      font-weight: 900;
      background: #fff;
    }
...
export function openAdminPricingSimulator() {
      ...
      <div class="admin-pricing-simulator__body">
        <section class="admin-pricing-simulator__section">
          <div class="admin-pricing-simulator__section-head">
            <h3>הוצאות שכר מדריך</h3>
            <small>הוצאה פנימית מחושבת · ניתן לערוך לצורך הסימולציה</small>
          </div>
          ...
          <div class="admin-pricing-simulator__assumptions">
            <div class="admin-pricing-simulator__assumptions-title">
              ...
              <span>ערכי ברירת המחדל קבועים בפתיחה, אך ניתנים לשינוי זמני</span>
            </div>
  ...
  const getPricingConfig = (values) => ({
    instructorPrice: Number(values.instructorPrice) || 0,
  ...
  const renderCalculations = () => {
    ...
    if (!wageComplete) {
      clearResults('יש להשלים הוצאות שכר מדריך');
      return;
    }
    if (!pricingComplete) {
    ...
    groups.forEach((group, index) => {
      ...
      const names = ['finalPrice', 'minimumPrice', 'profit', 'margin', 'instructorPrice', 'studentPrice', 'commission', 'instructorWage', 'venueCost', 'totalExpenses'];
