...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    .admin-staff-messages-shell {
      width: min(1120px, calc(100% - 40px));
      margin: 0 auto;
      padding: 28px 0 44px;
    }
    .admin-staff-messages-head {
      display: flex;
      align-items: center;
    ...
    .admin-staff-messages-head__title { min-width: 0; }
    .admin-staff-messages-head h1 {
      margin: 0;
      font-size: 26px;
      line-height: 1.25;
      font-weight: 850;
    }
    .admin-staff-messages-head p {
      margin: 6px 0 0;
      color: #64748b;
      font-size: 14px;
    }
    .admin-staff-messages-actions {
      display: flex;
      align-items: center;
      gap: 9px;
      flex-wrap: wrap;
    ...
    .admin-staff-messages-btn--danger { color: #b42318; border-color: #f1c5c2; }
    ...
    .admin-staff-messages-table__title {
      max-width: 340px;
      font-weight: 800;
    ...
    .admin-staff-messages-form__head {
...
function scheduleLabel(message) {
  ...
  if (message?.schedule_type === 'monthly') {
    return `בכל חודש, ביום ${Number(message.monthly_day)} בשעה ${time}`;
  }
  const date = normalize(message?.scheduled_date);
  return `${date || '—'} בשעה ${time}`;
}

function statusBadge(message) {
  if (!message?.is_active) {
    return '<span class="admin-staff-messages-badge admin-staff-messages-badge--inactive">מבוטלת</span>';
  }
  if (message?.schedule_type === 'monthly') {
    return '<span class="admin-staff-messages-badge">פעילה · חודשית</span>';
  }
  ...
  const values = Object.fromEntries(currentParts.map((part) => [part.type, part.value]));
...
function rowsHtml() {
  ...
  if (!messages.length) {
    return '<div class="admin-staff-messages-empty">עדיין לא נוצרו הודעות. לחצו על „הודעה חדשה” כדי להתחיל.</div>';
  }
  ...
  return `
    <div class="admin-staff-messages-table-wrap">
      <table class="admin-staff-messages-table">
        <thead><tr><th>הודעה</th><th>מועד</th><th>חשיבות</th><th>סטטוס</th><th>פעולות</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>
  `;
  ...
  root.innerHTML = `
    <div class="admin-staff-messages-shell">
      <header class="admin-staff-messages-head">
        <div class="admin-staff-messages-head__title">
          <h1>הודעות</h1>
      ...
      <section class="admin-staff-messages-panel" aria-label="רשימת הודעות">
        ${rowsHtml()}
      </section>
    </div>
  `;
    ...
    <div class="admin-staff-messages-form-overlay" data-message-form-overlay>
      <form class="admin-staff-messages-form" data-message-form>
        ...
        <div class="admin-staff-messages-form__body">
          ...
          <div class="admin-staff-messages-audience">קהל יעד: כל העובדים ללא מדריכים</div>
...
async function deleteMessage(id) {
  const message = messages.find((item) => normalize(item.id) === normalize(id));
  if (!message) return;
  if (!window.confirm(`למחוק את ההודעה „${message.title}”?`)) return;
...
function bindManagerActions(root) {
  ...
  root.querySelectorAll('[data-message-delete]').forEach((button) => {
    button.addEventListener('click', async () => {
      button.disabled = true;
      try { await deleteMessage(button.dataset.messageDelete); }
      catch (error) {
...
async function loadMessages() {
  ...
  const { data, error } = await supabase
    .from(MESSAGE_TABLE)
    .select('id,title,body,schedule_type,scheduled_date,monthly_day,scheduled_time,importance,audience,active_from,is_active,created_by,created_at,updated_at')
